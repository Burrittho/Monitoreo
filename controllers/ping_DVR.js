...
const logger = require('../utils/logger').child({ component: 'monitor_ping_dvr' });
...
async function obtenerIPs() {
    ...
    try {
        ...
        logger.error({ error: err }, 'Error al obtener IPs DVR');
        return [];
    } finally {
...
async function hacerPing(ips) {
    return new Promise((resolve) => {
        ...
        exec(comando, (error, stdout, stderr) => {
            ...
            if (error) {
                logger.warn({ error: { message: error.message } }, 'fping DVR finalizó con advertencia');
            }
...
async function guardarPingsEnLote(resultados) {
    ...
    try {
        ...
        if (ips.length > 0) {
            ...
            const [ipRows] = await conn.query(`SELECT id, ip FROM ips_dvr WHERE ip IN (${placeholders})`, ips);
        ...
        resultados.forEach((resultado) => {
            ...
            if (ipId) {
                ...
                logger.warn({ ip: resultado.ip }, 'IP DVR no encontrada en base de datos');
            }
        });
        ...
        if (conn) {
            try {
                ...
                logger.error({ error: rollbackErr }, 'Error en rollback DVR');
            }
        }
        logger.error({ error: err }, 'Error al guardar pings DVR en lote');
        throw err;
    } finally {
        if (conn) {
            try {
                ...
                logger.error({ error: releaseErr }, 'Error al liberar conexión DVR');
            }
        }
    }
}
...
async function iniciarPings_dvrContinuos() {
    logger.info({ intervalMs: 1000 }, 'Servicio de monitoreo DVR iniciado');
    ...
    setInterval(async () => {
        ...
        try {
            ...
            logger.error({ error: err }, 'Error durante el ciclo de ping DVR');
        }
    }, 1000);
}
