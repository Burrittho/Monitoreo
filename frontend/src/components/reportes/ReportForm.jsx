...
export default function ReportForm({ sucursales, form, setForm, providersSucursal, selectedProviderAccount, createMsg, creating, onSubmit, onProviderSelect }){
  return (
    <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700">Sucursal *</label>
        <select 
        ...
        >
          ...
          {sucursales.map(s=> (
            <option key={s.id||s.sucursal_id} value={s.id||s.sucursal_id}>
      ...
      <div>
        ...
        <select 
          ...
          className="block w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed" 
          ...
          required
        >
          <option value="">{providersSucursal.length ? 'Selecciona proveedor' : 'Selecciona sucursal primero'}</option>
          {providersSucursal.map(p=> (
            <option key={`${p.proveedor}-${p.tipo}`} value={p.proveedor}>
              {p.proveedor} ({p.tipo})
            </option>
          ))}
        </select>
      ...
      <div>
        <label className="block text-sm font-medium mb-2 text-gray-700">Número Reporte</label>
        <input 
          className="block w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" 
      ...
      <div className="md:col-span-2 lg:col-span-5">
        <label className="block text-sm font-medium mb-2 text-gray-700">Notas técnicas</label>
        <textarea 
          rows={3} 
          className="block w-full px-3 py-2 text-sm border border-gray-300 rounded-md bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none" 
      ...
      <div className="md:col-span-2 lg:col-span-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-gray-200">
        {createMsg.text && (
