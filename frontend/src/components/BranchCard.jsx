...
export default function BranchCard({ result }) {
  ...
  const handleClick = (e) => {
    if (success) {
      // Si está online, abrir en nueva pestaña
      window.open(`https://${result.ip}:4434`, '_blank');
    } else {
      // Si está offline, navegar internamente
      e.preventDefault();
      window.location.href = `/analytics/${result.id || result.ip}`;
    }
  }

  return (
    <div 
      className={`min-w-[140px] sm:min-w-[160px] rounded-xl p-4 shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 border-2 hover:delay-300 cursor-pointer ${statusClass}`}
      onClick={handleClick}
    >
